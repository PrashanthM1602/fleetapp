...
def login_view(request):
    if request.method == "POST":
        ...
        try:
            ...
            user, _ = User.objects.get_or_create(username=driver.driver_code)

            refresh = RefreshToken.for_user(user)

            request.session['jwt_token'] = str(refresh.access_token)
            request.session['driver_id'] = driver.id
            request.session['driver_name'] = driver.driver_full_name   # ✅ FIXED
...
def driver_profile_view(request):
    ...
    response = requests.get(
        "https://fleetapp-jym7.onrender.com/api/driver-profile/",  # ✅ FIXED
        headers=headers
    )
